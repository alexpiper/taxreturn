...
#'
#'
search_bold <- function(x, marker = "COI-5P", quiet = FALSE, output = "h",
...
#'
#' @examples
fetch_bold <- function(x, marker = "COI-5P", output = "gb-binom", quiet = FALSE, db=NULL, update=NULL) {
  # function setup
...
fetch_seqs <- function(x, database, marker = NULL, downstream = FALSE,
                       ...
                       force=FALSE, multithread = FALSE, quiet = TRUE, progress=FALSE, db=NULL) {
  ...
  if (database %in% c("genbank", "nuccore")) {
    ...
    res <- furrr::future_map(
      bold_taxon, search_bold, marker = marker, db=db,
...
#'
#' @examples
fetch_seqs2 <- function(x, database, marker = NULL, downstream = FALSE,
                        output = "gb-binom", min_length = 1, max_length = 2000,
                        subsample=FALSE, chunk_size=NULL, db=NULL,
                        multithread = FALSE, quiet = FALSE, progress=FALSE) {
  # function setup
  time <- Sys.time() # get time

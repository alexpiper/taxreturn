...
#' @param get_ncbi_lineage Get full taxonomic lineage using reformat_hierarchy if not already present
#' @param db A database file generated by `get_ncbi_taxonomy` or `get_ott_lineage`. Required if get_ncbi_lineage is TRUE
#' @param quiet Whether progress should be printed to console
#'
#' @return
#' @export
...
train_idtaxa <- function(x, maxGroupSize=10, maxIterations = 3,  allowGroupRemoval = TRUE, orient=FALSE,
                         get_ncbi_lineage=FALSE, db = NULL, quiet = FALSE) {
  time <- Sys.time() # get time

  #Reformat to complete taxonomic hierarchy
  if(get_ncbi_lineage & !is.null(db)){
    seqs <- taxreturn::reformat_hierarchy(x, db=db, quiet=FALSE)
  } else if(get_ncbi_lineage &  is.null(db)){
    stop("If get_ncbi_lineage is TRUE, a db needs to be provided")
  } else  (seqs <- x)

...
#' @param check_indels Whether sequences with insertions or deletions which arent in multiples of 3 should be removed from output. Useful for coding loci such as COI but should not be used for non-coding loci. Default is FALSE.
...
#' @param maxNs The max number of ambiguous N bases allowed before a sequence is removed.
...
#'
#' @examples
map_to_model <- function(x, model, minscore = 100, kmer_threshold=0.3, shave = TRUE, check_indels = FALSE, extra=NA, maxNs = Inf,
  ...
  if(length(short_seqs) > 0){
    short_seqs <- kmer_screen(short_seqs, model, threshold=kmer_threshold, quiet= quiet)
  }

  # Split the long sequences into chunks and conduct kmer screen on each chunk
  if(length(long_seqs) > 0){
    long_seqs <- long_seqs %>%
      furrr::future_map(subset_long_seq, model=model, split_length = model$size, threshold=kmer_threshold,
...
#'
#' @examples
kmer_screen <- function(seqs, model, threshold = 0.3, quiet=FALSE){
  # Check inputs
  if(!class(model) == "PHMM") {
  ...
  mbed_dist <- kmer::mbed(c(seed_seq, seqs), seeds = 1, k = 5, residues = NULL, gap = "-",
                          counts = FALSE)[,]
...
#'
#' @examples
closest_seq <- function(seqs, model, threshold, quiet=FALSE){
  # Check inputs
  if(!class(model) == "PHMM") {
    stop("Model must be of class PHMM")
  }
...
split_seqs <- function(x, split_length){
...
subset_long_seq <- function(x, model, split_length, threshold, quiet){
  # Split sequences into chunks
  splits <- split_seqs(x, split_length=split_length)
  if(!is.list(splits))(return(x))

  # find chunk with closest kmer distance
  min_split <- closest_seq(splits, model = model, threshold = threshold, quiet = quiet)
